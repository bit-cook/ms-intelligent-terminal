...
class TerminalCoreUnitTests::ShellIntegrationTests final
{
    ...
    TEST_METHOD(FindBlock_OrphanOpenMarker_ReturnsMarkerLineOnly);
    ...
    TEST_METHOD(Uninstall_RefusesToStripOrphanOpenMarker);
...
void ShellIntegrationTests::FindBlock_OrphanOpenMarker_ReturnsMarkerLineOnly()
{
    std::string content = "before\n";
    const auto blockStart = content.size();
    content += std::string{ kShellIntegrationBlockOpenMarker };
    const auto markerEnd = content.size();
    content += "\n$__it_si = 'leaked'\n. $__it_si\n";

    const auto [s, e] = FindShellIntegrationBlock(content);
    VERIFY_ARE_EQUAL(blockStart, s);
    VERIFY_ARE_EQUAL(markerEnd, e, L"Orphan-marker range must NOT engulf surviving body lines");
}
...
void ShellIntegrationTests::Install_OverwritesOrphanOpenMarker()
{
    ...
    // The leaked $__it_si lines from the corrupted block previously survived
    // because the next Install would only replace the orphan marker line.
    // This is documented behavior — we are intentionally NOT asserting they
    // are removed. We only assert no DUPLICATION occurred.
}
...
void ShellIntegrationTests::Uninstall_RefusesToStripOrphanOpenMarker()
{
    const auto profile = _ProfilePath();
    std::string content = "Write-Host 'pre'\n";
    content += std::string{ kShellIntegrationBlockOpenMarker };
    ...
    const auto r = Uninstall(profile.wstring());
    VERIFY_IS_TRUE(r.success);
    VERIFY_IS_TRUE(r.alreadyInstalled, L"Orphan marker must be left for next Install to overwrite");
    VERIFY_ARE_EQUAL(content, _ReadFile(profile), L"File contents must be untouched");
}
